...
public:
  ...
  Marco(): r(1.0), R(1.5), h(0.7), osc(0.1) {}
  ...
  void init_border()
  {
	...
	for (j=0;j<4;j++)
	  {
	    ...
	    p[i][j] = new_vertex(border_function(t));
	    p[i][j]->border=t;
	    if (j>0)
    ...
    q = new_vertex(vector3(0,0,0));
    for (j=0;j<4;j++)
      {
	quadr(p[2*((j+1)%2)][j],p[2*((j+1)%2)][(j+1)%4],p[1][(j+1)%4],p[1][j]);
	new_triangle(q,p[1][j],p[0][j]);
	new_triangle(q,p[1][j],p[2][j]);
	new_triangle(q,p[2*(j%2)][j],p[2*(j%2)][(j+1)%4]);
      }
  }

  void init_border_bis()
  {
    int i,j;
    vertex *p[15];
    ...
    for (i=0;i<9;i++)
      {
	p[i]=new_vertex(border_function(2.0*M_PI/3.0 * i));
...
static bool initializer = registry_function<Marco>("marco");
